  ...
  onJoin(client: Client, options: any) {
    try {
      ...
      if (this.gameLobby[num].length == num) {
        ...
        let g = new Game(this.gameLobby[num], this);
        let temp = g.players;
        let j = -1;
        g.players = [];
        for (let i = 0; i < 4; i++) {
      ...
      if (!this.gameOverIndex) {
        ...
        if (this.playerCount > 1) {
          ...
          if (this.players != null) {
            ...
            //     if (client.id == this.players[i].sock.id) {
            //       playerColor = this.players[i].playerColor;
            //       this.winnerUserId[this.playerCount] = this.players[i].userId;
            //       this.winnerAvatars[this.playerCount] = this.players[i].avatar;
