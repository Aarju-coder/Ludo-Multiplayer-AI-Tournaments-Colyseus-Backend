...
export class ludo3PlayerLobbyRoom extends Room<State> {
  /**
  ...
  onJoin(client: Client, options: any) {
    ...
    // // console.log( "another roomId 2", scoreManager.getRoom() );
    // // var key = scoreManager.getRoom();
    // // key.broadcast("broadcasted", {roomId : key.roomId});
    // var rooms = scoreManager.getRoomByTournment('123');
    // console.log("rooms map -> ",rooms);

    //scoreManager.broadcast("broadcasted", {roomId: scoreManager.showkey}, key)
    ...
    try {
      ...
      this.playerCount++;
      console.log("players in Lobby", this.playerCount);
      this.clientsArray[this.playerCount] = client;
      let num = parseInt(options.playerCount);
      // this.numClientsToMatch = num;
  ...
  recreateGroups() {
    ...
    this.matchingGroups.forEach((currentGroup) => {
      ...
      if (currentGroup.clients.length == this.numClientsToMatch) {
        ...
        totalPoints = 0;
        this.playerCountincurrentGroup = 0;
        this.playerCount = 0;
        this.clientsArray = [];
        console.log("currentGroup Length ", currentGroup.clients.length);
        console.log("Number of clients ", this.numClientsToMatch);
  ...
  onLeave(client) {
    try {
      ...
      console.log("error in three player lobby -> ", e);
    }
  }
