...
export class tournamentSingletonClass {
  ...
  async saveWinnerDb(winner, t_id, round) {
    try {
      ...
      let entries = [];
      let winners = [];
      console.log("winner before and after each round", winner);
      if (!winner.length) {
        for (const [key, value] of this._roomsCreatedForTournament) {
          if (key.startsWith(t_id + "_" + round)) {
            let roomObject = this._roomsCreatedForTournament.get(key);
      ...
      console.log("saving winners param ", JSON.stringify(param));
  ...
  public arrangeRoom(value: any, t_id: string, tournamentStartTime: any): void {
    ...
    console.log(value.length, " user length & round ", round);
    if (value.length == 1) {
      ...
      for (const [key, room] of this.lobbys) {
        if (key.startsWith(t_id)) {
          room.broadcast("tournamentEndedOnePlayerJoin", {
            winnerId: value[0].userId,
            rank1: value[0].rank1,
            rank1Avatar: value[0].rank1Avatar,
          });
        }
      }
      ...
      console.log("roomsCreated -> ", roomsCreated);
      ...
      if (round > 1) {
        setTimeout(() => {
          for (const [key, value] of this.lobbys) {
            if (key.startsWith(t_id)) {
              console.log("broadcast to lobby new round start");
